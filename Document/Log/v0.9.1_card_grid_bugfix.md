...
### Bug 5: Card Grid 的 Reference 渲染不整齐

**现象**:
- Reference 区块排版混乱，不如原来整齐
- 长链接直接裸露显示，导致折行观感差

**根因**:
1. 参考文献样式类（`detail-reference-*`）在 `styles.css` 中缺失，导致回退到默认样式。  
2. 模板把 URL 以裸文本追加在引用后（`... text + full url`），长链接会拉乱排版。

**修复方式**:
- 在 `css/styles.css` 与 `docs/css/styles.css` 增补并统一：
  - `.detail-rich-text--formatted`
  - `.detail-subtitle`
  - `.detail-table*`
...
- 在 `js/templates.js` 与 `docs/js/templates.js` 调整 `renderCardReferences()`：
  - 不再显示裸 URL
  - 有 URL 时改为“引用文本本身可点击”

### Bug 6: Card Grid 引用点击应跳转到底部总 Reference

**需求更正**:
Card Grid 的引用点击目标应优先跳转到页面底部的总 `Reference` 区，而不是仅作为外链打开。

**修复方式**:
- 在 `js/templates.js` 与 `docs/js/templates.js` 增加 `resolveFooterReferenceId()`：
  - 优先使用 `entry.id/refId`
  - 否则根据 `url` 在 `index.footer.reference.items` 中匹配
  - 再按 `text` 进行兜底匹配
- `renderCardReferences()` 在能解析到 `refId` 时，渲染为：
  - `class="detail-reference-link ref-cite"`
  - `data-ref-id="{id}"`
  - `href="#ref-{id}"`

这样会复用现有的 `.ref-cite` 点击处理器，平滑滚动并高亮到底部对应条目；无法匹配到底部编号时仍保留外链兜底。
...
## v0.9.6 Renewable Quiz 题库补充（8题）

### 变更内容

在 `Renewable` 模块补充完整 quiz 数据，新增 8 道选择题（含 `question/options/answer/explanation`），覆盖：
...
- 水电站 penstock 概念
- 2010 年水电最大生产国（China）
- 发电机输出 DC 所需 split ring commutator
- 感应电动势峰值条件
- 地热主要热源（放射性衰变）

### 实现方式

1. 按现有 quiz 引擎数据格式新增 `questions` 数组。
2. 保持 `answer` 使用 0-based 下标，确保与 `quiz.js` 判题逻辑兼容。
3. 为每题补充 explanation，便于前端答题后即时反馈展示。
...
- 前端渲染读取 `./material.json`，因此该变更已直接同步到可视化答题界面。

---

## v0.9.7 Renewable Quiz 文案回调（保留原文）

### 变更内容

根据最新要求，对 `Renewable` 的 8 道 quiz 题目文案进行回调：以“尽可能保留原文”为原则，不改语义与措辞，仅做 quiz 数据结构适配与必要排版。

### 实现方式

1. 保留原题干与选项文本；
2. explanation 回调为原始描述风格，不再做额外润色；
3. 继续使用 `answer` 下标映射正确选项，确保前端判题逻辑不变。
